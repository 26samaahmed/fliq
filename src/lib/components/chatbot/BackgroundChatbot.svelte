<script lang="ts">
	...
	// ✅ FIX: allow null (prevents "used before assigned" TS error)
		...
		if (len > processedExternal) {
			for (let i = processedExternal; i < len; i++) {
				...
				if ('imageBase64' in msg) {
					onImageUpdate?.(msg.imageBase64, msg.mimeType);
				}
			}
	...
	async function sendMessage() {
		...
		try {
			if (hasPhotos) {
				...
				if (firstError?.error) {
					messages.push({ role: 'bot', text: firstError.error });
					isLoading = false;
					await tick();
					scrollToBottom();
					return;
				...
				messages.push({
					role: 'bot',
					text: 'Of course! Please check the strip with the updated background.'
				});
			} else {
				const res = await fetch('/api/background-edit', {
					method: 'POST',
...
>
	...
	<div class="p-4 border-t border-gray-200 bg-white flex gap-3 items-center">
		<input
			...
			placeholder={disabled ? 'Host is customizing...' : 'Describe the background...'}
