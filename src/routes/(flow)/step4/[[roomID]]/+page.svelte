<script lang="ts">
	...
	let roomID = $derived(
		page.params.roomID || (typeof window !== 'undefined' ? sessionStorage.getItem('roomID') : null)
	);
...
>
	...
	<div class="mt-4">
		<div class="flex flex-col sm:flex-row items-center justify-between mb-2">
			...
			<h1 class="text-lg sm:text-2xl text-white text-center flex-1">Take Your Photos</h1>
		...
		{#if isCapturing}
			<p class="text-center text-white/80 text-base sm:text-lg mt-4 max-w-2xl mx-auto">
				Photo {photoCount} of {TOTAL_PHOTOS}
			</p>
		{:else}
			<p class="text-center text-white/80 text-base sm:text-lg mt-4 max-w-2xl mx-auto">
